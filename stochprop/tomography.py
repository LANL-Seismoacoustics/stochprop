...
def find_arrivals(ranges, azimuths, atmosphere_state, zonal_coeffs, merid_coeffs, incl_min=0.5, incl_max=45.0, incl_step=0.1, cpu_cnt=1):
    ...
    for az in np.unique(azimuths):
        if cpu_cnt > 1:
            subprocess.call("mpirun -np " + str(cpu_cnt) + " infraga-accel-2d -prop temp.met azimuth=" + str(az) + " bounces=0 incl_min=" + str(incl_min) + " incl_max=" + str(incl_max) + " incl_step=" + str(incl_step) + " calc_amp=false > /dev/null", shell=True)
        else:
            subprocess.call("infraga-2d -prop temp.met azimuth=" + str(az) + " bounces=0 incl_min=" + str(incl_min) + " incl_max=" + str(incl_max) + " incl_step=" + str(incl_step) + " calc_amp=false > /dev/null", shell=True)
        ...
        subprocess.call("rm atmo.dat temp.results.dat", shell=True)
    subprocess.call("rm temp.met", shell=True)
...
def jacobian(observations, src_loc, atmosphere_state, zonal_coeffs, merid_coeffs, incl_min=0.5, incl_max=45.0, incl_step=0.1, coeff_eps=0.5, cpu_cnt=1, default_err=1.0e7):
    ...
    for n_coeff in range(coeff_cnt):
        ...
        arrivals_up = find_arrivals(uniq_ranges, uniq_azimuths, atmosphere_state, zonal_coeffs_up, merid_coeffs_up, incl_min=incl_min, incl_max=incl_max, incl_step=incl_step, cpu_cnt=cpu_cnt)
        arrivals_dn = find_arrivals(uniq_ranges, uniq_azimuths, atmosphere_state, zonal_coeffs_dn, merid_coeffs_dn, incl_min=incl_min, incl_max=incl_max, incl_step=incl_step, cpu_cnt=cpu_cnt)
