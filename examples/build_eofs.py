...
if __name__ == '__main__':
    ...
    # ######################### #
    if os.path.isfile("eofs/" + loc_id + "-singular_values.dat"):
        print("eofs/" + loc_id + "-singular_values.dat exists" + '\n\t' + "Skipping construction of EOFs")
    else:
        ...
        print("Computing SVD...")
        eofs.compute_svd(A, z0, "eofs/" + loc_id)

    # ################################### #
    #  Compute EOF coefficients for each  #
    #     month and build season stats    #
    # ################################### #
    if os.path.isfile("coeffs/" + loc_id + "_12-coeffs.npy"):
        print("coeffs/" + loc_id + "_12-coeffs.npy exists" + '\n\t' + "Skipping calcuation of EOF coefficients")
    else:
        ...
        for m in range(12):
            ...
            coeffs = eofs.compute_coeffs(Am, zm, "eofs/" + loc_id, "coeffs/" + loc_id + "_{:02d}".format(m + 1), eof_cnt=100, pool=pl)
        pl.close()
        pl.terminate()

    if os.path.isfile("coeffs/" + loc_id + "-overlap.npy"):
        print("coeffs/" + loc_id + "-overlap.npy exists" + '\n\t' + "Skipping calculation of seasonality")
    else:
        coeffs = [0] * 12
        for m in range(12):
            coeffs[m] = np.load("coeffs/" + loc_id + "_{:02d}-coeffs.npy".format(m + 1))

        overlap = eofs.compute_overlap(coeffs)
        np.save("coeffs/" + loc_id + "-overlap", overlap)

    overlap = np.load("coeffs/" + loc_id + "-overlap.npy")
    eof_weight = np.loadtxt("eofs/" + loc_id + "-singular_values.dat")[:, 1]
    overlap_proj = np.average(overlap, weights=eof_weight[:overlap.shape[0]], axis=0)

    dist_mat = -np.log(overlap_proj)
    np.fill_diagonal(dist_mat, 0.0)
    hjl.cluster(dist_mat, clustering_thresh, show_result=True, file_id="coeffs/" + loc_id + "-clustering")
    ...
    plt.savefig("coeffs/" + loc_id + "-seasonality.png", dpi=300)
